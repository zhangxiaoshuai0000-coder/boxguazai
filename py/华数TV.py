...
class Spider(Spider):
    ...
    def parse_category_video(self, vod):
        # 修复：使用多种图片字段，确保封面能正确显示
        pic = vod.get('hPic') or vod.get('vPic') or vod.get('poster') or ''
        # 修复：确保标题不为空，避免显示 "null"
        title = vod.get('title') or ''
        return {
            "vod_id": f"{vod['nodeId']}@{vod['newsId']}",
            "vod_name": title,
            "vod_pic": pic,
            "vod_year": vod.get('pubTime') or '',
            "vod_remarks": vod.get('episodeDesc') or ''
        }
    ...
    def detailContent(self, ids):
        ...
        title = data_data.get('title') or ''

        content = self.build_content(data)
        # 修复：确保所有字段都不会返回 null
        director = self.extract_detail_field(data, 'director') or ''
        actor = self.extract_detail_field(data, 'actor') or ''
        remarks = self.extract_detail_field(data, 'episodeDesc') or ''
        year = self.extract_detail_field(data, 'pubTime') or ''
        area = self.extract_detail_field(data, 'countryTag') or ''
        bofang = self.build_play_url(data)

        # 修复：获取封面图片，确保历史记录封面显示正确
        pic = ''
        if data_data:
            # 尝试多种图片字段
            pic = data_data.get('hPic') or data_data.get('vPic') or data_data.get('poster') or data_data.get('coverImg') or ''
    ...
    def build_content(self, data):
        return data.get('data', {}).get('newsAbstract') or ''
    ...
    def build_video_data(self, did, director, actor, remarks, year, area, content, bofang, pic=''):
        return {
            "vod_id": did,
            "vod_director": director,
            "vod_actor": actor,
